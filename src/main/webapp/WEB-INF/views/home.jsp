...
<%@ page session="true" %>
    ...
    <div class="w3-col l8 s12">
     <!-- Blog entry -->
     <c:forEach items="${Home_firstList}" var="Home_firstList" varStatus="loop">
     
      <div class="w3-container w3-white w3-margin w3-padding-large">
        <div class="w3-center">
          <h3>${Home_firstList.TITLE}</h3>
         <span class="w3-opacity">${resultData.UPLOAD_DATE}</span>
        </div>

        <div class="w3-justify">
          <img src="<c:url value='/resources/images/${Home_firstList.IMG_NAME}'/>" alt="image" style="width:100%" class="w3-padding-16">
			<p>${Home_firstList.CONTENTS}</p>
...
<!--               <div class="w3-col l10 m9"> -->
              ...
              <c:if test = "${inhog.BOARD_SEQ eq Home_firstList.BOARD_SEQ}">
              	<hr style = "margin-top: 0; margin-bottom: 0">
                <h4>${inhog.ID}  <a href = "<c:url value ='/menu/shoes/delete_comments_register?BOARD_SEQ=${Home_firstList.BOARD_SEQ}&CATEGORY_SEQ=${Home_firstList.CATEGORY_SEQ}&COMMENT_SEQ=${inhog.COMMENT_SEQ }'/>"><button class="w3-button w3-right w3-tiny w3-padding-small w3-white w3-border">x</button></a></h4>
                <p>${inhog.CONTENTS}</p>
              </c:if>
            </c:forEach>
            </c:forEach>
