...
<!--               <div class="w3-col l10 m9"> -->
              ...
              <c:if test = "${inhog.BOARD_SEQ eq resultData.BOARD_SEQ}">
                ...
                <h4>${inhog.ID}  <a href = "<c:url value ='/menu/news/delete_comments_register?BOARD_SEQ=${resultData.BOARD_SEQ}&CATEGORY_SEQ=${resultData.CATEGORY_SEQ}&COMMENT_SEQ=${inhog.COMMENT_SEQ }'/>"><button class="w3-button w3-right w3-tiny w3-padding-small w3-white w3-border">x</button></a></h4>
                <p>${inhog.CONTENTS}</p>
              </c:if>
            </c:forEach>
            </c:forEach>
...
<form class="form-horizontal" role="form" method="post" action="<c:url value ='/menu/news/insert_comments_register?BOARD_SEQ=${resultData.BOARD_SEQ}&CATEGORY_SEQ=${resultData.CATEGORY_SEQ}&ID=${principalName}'/>">
